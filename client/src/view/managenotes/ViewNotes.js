...
const ViewNotes = () => {
      ...
      await fetch(`${url}noteslist`, {
        ...
        .then((data) => {
          if (data.status === 200) {
            ...
            setNotes(data.data.filter((note) => note.deletedNote === 0));

            // console.log(data.data);

            //  ------For TableViewer ------
            const notesData = data.data
              .filter((note) => note.deletedNote === 0)
              .map((note) => ({
  ...
  const btnDeleteHandle = (deleteId, i) => {
    Swal.fire({
      title: "Are you sure?",
      text: "You are about to delete note. Are you sure you want to proceed?",
      icon: "warning",
  ...
  const handleDeleteNotes = async (deleteId, i) => {
    ...
    try {
      await fetch(`${url}noteslist/bin/${deleteId}`, {
        ...
        .then((data) => {
          if (data.status === 200) {
            triggerNotification("success", "Note deleted successfully.");

            //  ------For TableViewer ------
            const updatedNotes = [...notesRef.current];
            updatedNotes.splice(i, 1);
            setNotes(updatedNotes.filter((note) => note.deletedNote === 0));
  ...
  //----------------------------------------------- select / view single note -------------------------------------------------R
  const btnShowHandle = (id) => {
    const encodedId = window.btoa(encodeURIComponent(id));
    navigate(`/addnotes/${encodedId}`);
  };
  ...
  return (
    <>
      <Breadcrumb txt="Manage Notes" subpath={["View Notes"]} />

      <div className="col-xxl">
        <div className="card mb-4">
          <div className="card-header d-flex align-items-center  row flex justify-between">
            <h5 className="mb-0 col-sm-3 ">View Notes</h5>
            <button
              type="button"
              className="btn btn-primary col-sm-2 mr-2 mt-2"
              onClick={() => {
                navigate("/addnotes");
              }}
            >
          ...
          {loading ? (
            ...
            <div className="px-3 pb-3  table-responsive">
              {/* ------For TableViewer ------ */}
              <TableViewer rowData={rowData} colDefs={colDefs} onDelete={btnDeleteHandle} onView={btnShowHandle} />
              {/* ------For TableViewer ------ */}
            </div>
          )}
        </div>
